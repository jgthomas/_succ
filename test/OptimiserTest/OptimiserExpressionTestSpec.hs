...
module OptimiserTest.OptimiserExpressionTestSpec (optimiserExpressionTest) where
...
optimiserExpressionTest = hspec $ do
        describe "Optimise expressions to reduce code size" $ do
                ...
                it "Should pass back unchanged a binary schema with no optimisation support" $
                  ...
                  `shouldBe`
                  ExpressionSchema (BinarySchema SkipSchema SkipSchema Plus (LocalLabel 1) (LocalLabel 2))
                ...
                it "Should pass back unchanged a schema with no optimisation support" $
                  ...
                  `shouldBe`
                  ExpressionSchema (DereferenceSchema SkipSchema)
