...
import State.FuncState as FuncState
  ( checkVariable,
    getBreak,
    getContinue,
    parameterDeclared,
...
checkIfUsedInScope node@(DeclarationNode (VarNode name _) _ _ _) = do
  ...
  when (localDec || paramDec)
    $ throwError
    $ ScopeError (DoubleDeclaredNode node)
...
validateCall node@(FuncCallNode name _ _) = do
  ...
  unless (validSeq callee caller)
    $ throwError
    $ ScopeError (InvalidCallNode node)
...
checkIfFuncDefined node@(FunctionNode _ name _ _ _) = do
  ...
  when defined
    $ throwError
    $ ScopeError (DoubleDefinedNode node)
...
checkCountsMatch count node@(FunctionNode _ _ paramList _ _) =
  when (count /= length paramList)
    $ throwError
    $ ScopeError (MisMatchNode count node)
...
varExists node name = do
  ...
  when (var == NotFound)
    $ throwError
    $ ScopeError (UnrecognisedNode node)
